...
public class HomePage {
    ...
    public SearchResultsPage searchProduct(String productName) {
        try {
            // For empty search, return null to indicate no search performed
            if (productName == null || productName.trim().isEmpty()) {
                return null;
            }

            // Wait for search box and ensure it's ready
            WebElement search = wait.until(ExpectedConditions.elementToBeClickable(searchBox));
            
            // Clear using both clear() and JavaScript
            search.clear();
            ((JavascriptExecutor) driver).executeScript("arguments[0].value = '';", search);
            
            // Type the search term
            search.sendKeys(productName.trim());
